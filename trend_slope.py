...
def compute_trend_slope():
    ...
    try:
        ...
        last_slope = slope_df["Slope"].iloc[-1]  # Get the last computed slope
        #st.write(f"**Predicted Next Day Trend:
        if last_slope > 0:
            trend_prediction = "Uptrend Expected"
            st.write(f"**Predicted Next Day Trend:** {trend_prediction}")
        elif last_slope < 0:
            trend_prediction = "Downtrend Expected"
            st.write(f"**Predicted Next Day Trend:** {trend_prediction}")
        else:
            trend_prediction = "No Clear Trend ❓"
            st.write(f"**Predicted Next Day Trend:** {trend_prediction}")
