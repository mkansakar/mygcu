...
def calculate_bollinger_bands(data, column='Close', window=20):
    ...
    # Trend Prediction: Up (1) or Down (0)
    data['Trend'] = 0  # Default to Down
    data.loc[data[column] > upper_band, 'Trend'] = 1  # Uptrend
    data.loc[data[column] < lower_band, 'Trend'] = -1  # Strong Downtrend

    return rolling_mean, upper_band, lower_band, data['Trend']
...
def display_volatility_indicators():
    ...
    try:
        ...
        rolling_mean, upper_band, lower_band, trend = calculate_bollinger_bands(data, window=window_bb)

        def display_trend(data):
            if 'Trend' not in data.columns:
                st.write("Trend data is missing. Please run calculate_bollinger_bands first.")
                return

            latest_trend = data['Trend'].iloc[-1]  # Get the latest trend value

            if latest_trend == 1:
                return "Uptrend detected."
            elif latest_trend == -1:
                return "Strong Downtrend detected."
            else:
        ...
        trend_result = display_trend(data)  # Call function properly
        st.write(f"**Bollinger Bands Analysis: {trend_result}**") 
